...
export default function Register() {
  ...
  const verificationCodeInSession = sessionStorage.getItem('verification_code');
  ...
  // (has verification code or pending login context)
  const isFromVerificationFlow = Boolean(verificationCodeInSession || pendingLoginContext);
                ...
                <div>
                  ...
                  {isEmailJSConfigured() ? (
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-4 text-center">
                      {t('verificationCodeSentTo')} <span className="font-semibold">{verificationContext?.email || formData.email}</span>
                    </p>
                  ) : (
